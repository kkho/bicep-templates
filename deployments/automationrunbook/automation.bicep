...
resource automationAccountDiagnosticsLogging 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (!empty(loganalyticsWorkspaceId)) {
  name: 'diags'
  scope: automationAccount
  properties: {
    workspaceId: loganalyticsWorkspaceId
    logs: [
      for diagCategory in diagnosticCategories: {
...
resource schedules 'Microsoft.Automation/automationAccounts/schedules@2024-10-23' = [
  for schedule in schedulesToCreate: {
    ...
    properties: {
      startTime: dateTimeAdd(
        dateTimeAdd(tomorrow, 'PT${schedule.hour}H'),
        'PT${schedule.minute}M',
...
resource runbook 'Microsoft.Automation/automationAccounts/runbooks@2024-10-23' = if (!empty(runbookName)) {
  parent: automationAccount
...
resource automationJobs 'Microsoft.Automation/automationAccounts/jobSchedules@2024-10-23' = [
  for job in runbookJobSchedule: if (!empty(runbookName)) {
    parent: automationAccount
